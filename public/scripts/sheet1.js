...
//Info
{
  ...
  async function infoFocusOut(ev) {
    ...
    const label = $(
      `<label class="info-field" for="${forID}">${el.val()}</label>`
    );
...
function findAvatar() {
  ...
  const field = $(".attribute-status-container input:checked")
    .filter(function () {
...
//Attributes
{
...

    {
      ...
      if (attributeID === ATTRIBUTE_ID) {
        ...
        if (value !== newValue) {
          ...
          findAvatar();
          axios
            .post("/sheet/player/attributestatus", {
              attributeStatusID: ATTRIBUTE_STATUS_ID,
              checked: newValue,
            })
            .catch(showFailureToastMessage);
        }
      }
    }
  ...
  attributeContainer.find(".attribute-button").click(async (ev) => {
...
$(".attribute-status-container input").change(async (ev) => {
  ...
  findAvatar();
  try {
    await axios.post("/sheet/player/attributestatus", {
      attributeStatusID,
      checked,
    });
  } catch (err) {
...
const addEquipmentButton = $("#addEquipmentButton");
{
  ...
  addEquipmentButton.click(async (ev) => {
    ...
    try {
      const response = await axios.put("/sheet/player/equipment", {
        equipmentID,
      });
      ...
      addEquipmentButton.prop(
        "disabled",
        addEquipmentList.children().length === 0
      );
  ...
  socket.on("equipment added", (content) => {
    ...
    if (
      equipmentTable
        .find("tr")
        .filter((i, el) => $(el).data("equipment-id") === equipmentID).length >
        0 ||
...
async function deleteEquipmentClick(ev) {
  ...
  try {
    ...
    if (addEquipmentList.find(`option[value="${equipmentID}"]`).length === 0)
      addEquipmentList.append(
        $(`<option value="${equipmentID}">${equipName}</option>`)
      );
    row.remove();
    addEquipmentButton.prop("disabled", false);
  } catch (err) {
...
async function skillChange(ev) {
  ...
  try {
    const response = await axios.post("/sheet/player/skill", {
      skillID,
      value,
    });
...
const addItemList = $("#addItemList");
{
  ...
  socket.on("item added", (content) => {
    ...
    const name = content.name;
    if (
      itemTable.find("tr").filter((i, el) => $(el).data("item-id") === itemID)
        .length > 0 ||
  ...
  socket.on("item changed", (content) => {
    ...
    const row = itemTable
      .find("tr")
      .filter((i, el) => $(el).data("item-id") === itemID);
    if (row.length > 0) {
      if (name) row.find(".name").text(name);
    } else {
...
$(".extra-info-container textarea").change(async (ev) => {
  const extraInfoID = $(ev.target)
    .parents(".extra-info-container")
    .data("extra-info-id");
