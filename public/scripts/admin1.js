$(".player-container button.delete-player").click(async (ev) => {
  ...
  if (
    !confirm(
      `Tem certeza que deseja apagar o jogador ${name}? Essa ação é irreversível.`
    )
  )
    return;

  try {
    await axios.delete("/sheet/player", {
      data: { playerID: container.data("player-id") },
    });
    container.remove();
  } catch (err) {
...
$(".player-container button.create-shadow-player").click(async (ev) => {
  const container = $(ev.target).parents(".player-container");
  const name = container.find(".player-info").text().trim();

  if (
    !confirm(
      `Tem certeza que deseja criar um personagem alternativo para ${name}?`
    )
  )
    return;

  try {
    await axios.post("/register/shadow", {
      data: { playerID: container.data("player-id") },
    });
  } catch (err) {
    showFailureToastMessage(err);
...
$("#changeEnvironment").click(async (ev) => {
  ...
  try {
    await axios.post("/portrait/environment", { combat });
  } catch (err) {
    showFailureToastMessage(err);
  }
});
...
$("#hideShadows").click(async (ev) => {
  ...
  try {
    ...
    if (hidden)
      $('.player-container-wrapper[data-shadow-player-id="null"]').hide();
...
$("#rollGeneralDice").click(async (ev) => {
...
async function playerLineageChange(ev) {
  ...
  try {
    ...
    const img = $(
      `.player-container[data-player-id="${playerID}"] .avatar-container .lineage`
    );
    img
      .data("lineage", lineageID)
      .attr(
        "src",
        lineageID ? `/assets/lineages/frameless/${lineageID}/1.png` : ""
      );
  } catch (err) {
    showFailureToastMessage(err);
  }
...
async function onPlayerScoreChanged(ev) {
  ...
  const playerID = $(ev.target).parents(".player-container").data("player-id");
  try {
    await axios.post("/sheet/player/score", { value, playerID });
  } catch (err) {
    showFailureToastMessage(err);
  }
}
...
{
  const combatList = $("#initiativeList");
  Sortable.create(combatList[0], {
    animation: 150,
    onUpdate: (ev) => (currentIndex = ev.newIndex),
  });
  ...
  $(".initiative-button").click((ev) => {
    const children = combatList.children();
  ...
  function onAddToList(ev) {
    ...
    const span = $(
      `<span class="name acds-element acds-bottom-text text-center">${originBtn.text()}</span>`
    );
    const btn = $(`<button class="ms-2 btn btn-secondary btn-sm">-</button>`);
    const li = $(`<li data-player-id="${playerID}"></li>`).append(span, btn);

    detachedPlayerButtons.set(playerID, originBtn.detach());

    btn.click(() => {
      const playerBtn = detachedPlayerButtons.get(playerID);
      playerBtn.text(span.text());
      dropdown.append(playerBtn);

      combatList.children().eq(currentIndex).removeClass("active");
      ...
      combatList.children().eq(currentIndex).addClass("active");
    });

    combatList.append(li);
  }

  function npcChange(ev) {
    const playerId = $(ev.target).parents("li").data("player-id");

    combatList
      .find(`li[data-player-id="${playerId}"] span`)
      .text($(ev.target).val());

    $("#initiativeDropdown")
      .find(`button[data-player-id="${playerId}"]`)
      .text($(ev.target).val());
  }

  function npcRemove(ev) {
    const playerId = $(ev.target).parents("li").data("player-id");

    $("#npcList").find(`li[data-player-id="${playerId}"]`).remove();
    combatList.find(`li[data-player-id="${playerId}"]`).remove();
    $("#initiativeDropdown")
      .find(`button[data-player-id="${playerId}"]`)
      .remove();
  }
  ...
  $("#addNPC").click((ev) => {
    ...
    const id = Date.now();
    const name = `NPC ${list.children().length}`;

    const item = $(
      `<li data-player-id="${id}">
            <input class="name acds-element acds-bottom-text" value="${name}" onchange="npcChange(event)"></input>
            <input type="text" class="health acds-element acds-bottom-text text-center" value="0/0"></input>
            <button class="btn btn-secondary btn-sm" onclick="npcRemove(event)">-</button>
            </li>`
    );

    $("#initiativeDropdown").append(
      $(`
            <li>
                <button class="dropdown-item btn btn-dark" 
                data-player-id="${id}" onclick="onAddToList(event)">
                    ${name}
                </button>
            </li>
        `)
...
socket.on("attribute changed", (content) => {
  ...
  $(`.player-container[data-player-id="${playerID}"] 
    .player-attribute[data-attribute-id="${attrID}"]`).text(
...
socket.on("attribute status changed", (content) => {
  const container = $(
    `.player-container[data-player-id="${content.playerID}"]`
  );
  const array = container.data("attribute-status");

  const updatedAttrStatus = array.find(
    (attr) => attr.attribute_status_id === content.attrStatusID
  );
...
socket.on("equipment added", (content) => {
  ...
  $(
    `.player-container[data-player-id="${playerID}"] .player-equipment-table`
  ).append(row);
});
...
socket.on("equipment changed", (content) => {
  ...
  $(`.player-container[data-player-id="${playerID}"] 
    .player-equipment[data-equipment-id="${equipmentID}"] .using`).attr(
    "class",
    using ? "bi-check" : "bi-x"
  );
});
...
socket.on("item added", (content) => {
  ...
  row
    ...
    .text(name)
    .attr("title", `${description} (${quantity})`);

  $(
    `.player-container[data-player-id="${playerID}"] .player-item-table`
  ).append(row);
});
...
socket.on("item changed", (content) => {
  ...
  $(`.player-container[data-player-id="${playerID}"]
    .player-item[data-item-id="${itemID}"]`).attr(
    "title",
...
socket.on("dice result", (content) => {
  ...
  if (results.length > 1)
    msg.append(` Soma: 
    <span style='color:lightgreen;'>${results.reduce(
      (a, b) => a + b,
      0
    )}</span>.`);
...
function showAvatar(container, attrStatusID = 0) {
  ...
  avatarImage.attr(
    "src",
    `/avatar/${attrStatusID}?playerID=${playerID}&v=${Date.now()}`
  );
}
