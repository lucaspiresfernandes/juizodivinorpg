$('.player-container button.delete-player').click(async (ev) => {
	...
	if (
		!confirm(`Tem certeza que deseja apagar o jogador ${name}? Essa ação é irreversível.`)
	)
		return;
...
async function playerLineageChange(ev) {
	...
	try {
		...
		img
			...
			.attr('src', lineageID ? `/assets/lineages/frameless/${lineageID}/1.png` : '');
	} catch (err) {
		showFailureToastMessage(err);
	}
}
...

{
	...
	function npcChange(ev) {
		...
		combatList.find(`li[data-player-id="${playerId}"] span`).text($(ev.target).val());
	...
	function npcRemove(ev) {
		...
		$('#initiativeDropdown').find(`button[data-player-id="${playerId}"]`).remove();
	}
...
socket.on('equipment added', (content) => {
	...
	$(`.player-container[data-player-id="${playerID}"] .player-equipment-table`).append(
		row
	);
});
