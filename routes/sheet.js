...
router.get("/1", async (req, res) => {
  ...
  try {
      ...
      con("attribute")
        ...
        .join("player_attribute_status", (builder) =>
          builder
            ...
            .on(
              "player_attribute_status.player_id",
              "player_attribute.player_id"
            )
        )
        ...
        .then((attributesQuery) => {
          ...
          for (const attrQuery of attributesQuery) {
            ...
            attrQuery.coefficient =
              (attrQuery.value / attrQuery.total_value) * 100 || 0;
    ...
    const attributes = results[2].map(({ status, ...attr }) => {
      const st = status.map((stat) => {
        const link = results[1].find(
          (av) => av.attribute_status_id === stat.attribute_status_id
        )?.link;
        return { ...stat, link };
      });

      return { ...attr, status: st };
    });
      ...
      (async () => {
        ...
        await Promise.all(
          lineageNodes.map(async (node) => {
            const playerNode = playerNodes.find(
              (pNode) => pNode.index === node.index
            );
...
router.post("/player/attributestatus", jsonParser, async (req, res) => {
  ...
  try {
    ...
    io.to("admin").emit("attribute status changed", {
      playerID,
      attrStatusID,
      value,
    });
...
router.post("/skill", jsonParser, async (req, res) => {
  ...
  try {
    ...
    if (skillNames.spec_name)
      skillName = `${skillNames.spec_name} (${skillName})`;
...
router.post("/player/lineage", jsonParser, async (req, res) => {
  ...
  if (!playerID || req.body.lineageID === undefined)
    return res.status(401).send();
    ...
    await Promise.all([
      ...
      (async () => {
        const conqueredNodes = (
          await con("player_lineage_node")
            .select("index")
            .where("player_id", playerID)
        ).map((node) => node.index);
        const newlyConqueredSubsequentsNode = await con(
          "lineage_node_connection"
        )
          .select()
          ...
          .join("lineage_node", (builder) =>
            builder
              .on(
                ...
                "lineage_node_connection.lineage_id"
              )
...
router.get("/shadow", jsonParser, async (req, res) => {
  ...
  if (shadowPlayerID) {
    ...
    const lightPlayerID = (
      await con("player")
        .select("player_id")
        .where("shadow_player_id", playerID)
        .first()
    )?.player_id;
    if (lightPlayerID) {
    ...
    attributes.map((attr) => {
      ...
      const charValue =
        characteristics.find(
          (char) => char.characteristic_id === attr.characteristic_id
        )?.value || 0;
      const skillValue =
        skills.find((skill) => skill.skill_id === attr.skill_id)?.total_value ||
        0;
...
async function updateSkills(playerID, skills) {
  ...
  return await Promise.all(
    skills.map((skill) => {
      ...
      const extraValue =
        charValue + (classSkillIDs.includes(skillID) ? classBonus : 0);
